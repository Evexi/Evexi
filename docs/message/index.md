...
The Evexi Message API allows you to communicate between players that are assigned in the same group on the Evexi CMS that are part of the same network.
...
```typescript
const response = await Evexi.message.send('Hello World')
if (!response.success) {
